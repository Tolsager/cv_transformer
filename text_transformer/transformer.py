...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        queries = rearrange(queries, 'b s (nh hd) -> b s nh hd', hd=self.head_dim)
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_dim, max_seq_len=512):
        ...
        # multiply each position with -(math.log(10000.0) / embed_dim)
...
class TransformerClassifier(nn.Module):
    def __init__(self, embed_dim, num_heads, num_layers, max_seq_len,
        ...
        if self.pool == 'cls':
            ...
            self.cls_token = nn.parameter.Parameter(torch.zeros(1, 1, embed_dim), torch.float)
            max_seq_len +=1
    ...
    def forward(self, x):
        ...
        if self.pool == 'cls':
            ...
            cls_tokens = repeat(self.cls_token, 'b s e -> (repeat b) s e', repeat=batch_size)
            tokens = torch.concat([cls_tokens, tokens], dim=1)
        ################################################################
